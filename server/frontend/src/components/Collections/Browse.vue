  ...
  <div id="main" class="panel panel-default">
    <div class="panel-heading">
      <div>
        ...
        <form class="form-inline pull-right" style="transform: translateY(-22%);">
          <label for="rc_select">
            <i class="bi bi-funnel-fill"></i>
          </label>
          <select id="rc_select" class="form-control" @change="set_rc($event)">
            <option disabled selected value style="display:none;">!{ rc_name }!</option>
            <option v-for="entry in rc_list" :value="entry.id">!{ entry.description }!</option>
          </select>
        </form>
      </div>
    </div>
      ...
      <table class="table table-coverage">
        <tbody>
          <tr v-for="item in source">
            <td :id="item.idx" :class="['cov', 'cov-linenumber', item.lclass]"
                @click="line_click_handler(item.idx)">
              !{ item.idx }!
            </td>
            <td :class="['cov', 'cov-occurence', item.lclass]">
              <span class="label label-success" v-if="item.hits">
                <!-- FIXME -->
                !{ item.hits | formatNumber }!
              </span>
            </td>
            <td :class="['cov', 'cov-codeline', item.lclass]">
              <div v-html="highlight_code(item.line)">
                !{ highlight_code(item.line) }!
              </div>
            </td>
          </tr>
        </tbody>
    ...
    <template v-else>
      ...
      <line-chart style="height: 50vh;" v-if="chartdata"></line-chart>
        ...
        <tbody>
          <tr v-for="(value, path) in ordered_collection">
            <td @click="navigate(value)">
              <i :class="['bi', add_path_class(value)]"></i> <span :class="get_span_path_class(value)">!{ value.name
                }!</span>
            </td>
...
import {
  E_SERVER_ERROR, formatClientTimestamp,
} from "../../helpers";
...
export default {
    data() {
      return {
        ...
        rc_dropdown: false,
        rc_name: 'Report Configuration', // Placeholder value
        rc_id: null,
        rc_list: [],
    ...
    updated: function () {
      ...
      this.$nextTick(function () {
        ...
        let new_line = pmanager.get_value('s')
    ...
    methods: {
        ...
        fetch(this.apiurl(), {
          ...
          if ('source' in this.coverage) {
            ...
            for (let i = 0; i < lsource.length; i++) {
              ...
              psource.push({line: lsource[i], lclass: lclass, hits: lhits, idx: i})
            }
      ...
      get_report_metadata: function () {
        ...
        if (!ISDIFF) {
          ...
          ids = pmanager.get_value('ids')
          if (!ids) return;
        }

        return fetch(`${URLS['reports_api']}?coverage__ids=${ids}`, {
          method: 'GET',
          credentials: 'same-origin'
        }).then(response => {
          if (response.ok) {
          ...
          if (!ISDIFF && this.report_list) {
            let report = compute_report_description(this.report_list[0])
            document.title = `${report.description} - CovManager`
          } else {
            let new_labels = this.chartdata['labels']
            for (let i = 0; i < this.chartdata['ids'].length; ++i) {
              for (let report of this.report_list) {
                if (report['coverage'] == this.chartdata['ids'][i]) {
                  ...
                  this.chartdata['labels'][i] = this.wrap_text(report.description, 10)
                  break
                }
              }
            }
